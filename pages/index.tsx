...
export default function Home() {
  ...
  function handlePlay(song: any, url: string) {
    if (url !== '') {
      setName(song);
      setUrl(url);
    } else {
    ...
    <Layout title='Dashboard - Stir-Up Church Sermons'>
      ...
      <div className='mt-2 grid grid-cols-1 gap-4 min-[500px]:grid-cols-2 md:grid-cols-4'>
        {genre ? (
          genre
            .slice(0, 4)
            .map((item: any, index: number) => <GenreItem key={index} href={`?genre=${item.id}`} title={item.name} />)
        ) : (
          <>
              ...
              <SplideTrack>
                <SplideSlide>
                  <div className='grid grid-cols-1 gap-4 p-1 min-[500px]:grid-cols-2 md:grid-cols-3'>
                    {songs?.slice(0, 9).map((item: any, index: number) => (
                      <SongListItem
                        key={index}
                ...
                <SplideSlide>
                  <div className='grid grid-cols-1 gap-4 p-1 min-[500px]:grid-cols-2 md:grid-cols-3'>
                    {songs?.slice(9, 18).map((item: any, index: number) => (
                      <SongListItem
                        key={index}
                        href={`/dashboard/song/detail/${item.id}`}
                        imageSrc={item.cover_url}
              ...
              <SplideTrack>
                <SplideSlide>
                  <div className='grid grid-cols-1 gap-4 p-1 min-[500px]:grid-cols-2 md:grid-cols-3 xl:grid-cols-5'>
                    {albums?.slice(0, 5).map((item: any, index: number) => (
                      <AlbumItem
                        key={index}
                ...
                <SplideSlide>
                  <div className='grid grid-cols-1 gap-4 min-[500px]:grid-cols-2 md:grid-cols-3 xl:grid-cols-5'>
                    {albums?.slice(5, 10).map((item: any, index: number) => (
                      <AlbumItem
                        key={index}
                        href={`dashboard/album/detail/${item.id}`}
                        imageSrc={item.cover}
